...
export const Store = {
    ...
    get(key) {
        if(!Array.isArray(key)) {
            return AsyncStorage.getItem(key).then(values => { return JSON.parse(values); });
        } else {
     ...
     */
    push(key, value) {
        return AsyncStorage.get(key).then((currentValue) => {
            if (currentValue === null) {
                // if there is no current value populate it with the new value
                return AsyncStorage.save(key, [value]);
            }
            if (Array.isArray(currentValue)) {
                return AsyncStorage.save(key, [...currentValue, value]);
            }
